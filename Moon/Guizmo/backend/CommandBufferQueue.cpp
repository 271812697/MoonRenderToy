...
namespace MOON
{
	...
	void CommandBufferQueue::flush() noexcept {
		...
		auto begin = range.tail;
		auto end = range.head;
		assert(circularBuffer.empty());
		// size of the current buffer
		size_t const used = std::distance(static_cast<char const*>(begin), static_cast<char const*>(end));
	...
	void CommandBufferQueue::releaseBuffer(CommandBufferQueue::Range const& buffer) {
		size_t const used = std::distance(static_cast<char const*>(buffer.begin), static_cast<char const*>(buffer.end));
		std::lock_guard<std::mutex> const lock(mLock);
		mFreeSpace += used;
		mCondition.notify_one();
	}
