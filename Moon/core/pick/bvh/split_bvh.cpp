...
namespace MOON
{
	void SplitBvh::BuildImpl(bbox const* bounds, int numbounds)
	{
		...
		std::vector<FVector3> centroids(numbounds);
	...
	SplitBvh::SahSplit SplitBvh::FindObjectSahSplit(SplitRequest const& req, PrimRefArray const& refs) const
	{
		...
		// in this case
		FVector3 centroid_extents = req.centroid_bounds.extents();
		if (FVector3::Dot(centroid_extents, centroid_extents) == 0.f)
		{
			return split;
		}
	...
	SplitBvh::SahSplit SplitBvh::FindSpatialSahSplit(SplitRequest const& req, PrimRefArray const& refs) const
	{
		...
		// Extents
		FVector3 extents = req.bounds.extents();
		auto invarea = 1.f / req.bounds.surface_area();

		// If there are too few primitives don't split them
		if (FVector3::Dot(extents, extents) == 0.f)
		{
			return split;
		}
		...
		FVector3 origin = req.bounds.pmin;
		FVector3 binsize = req.bounds.extents() * (1.f / kNumBins);
		FVector3 invbinsize = FVector3(1.f / binsize.x, 1.f / binsize.y, 1.f / binsize.z);
		...
		for (int i = req.startidx; i < req.startidx + req.numprims; ++i)
		{
			...
			FVector3 firstbin = FVector3::Clamp(invbinsize * (primref.bounds.pmin - origin), FVector3(0, 0, 0), FVector3(kNumBins - 1, kNumBins - 1, kNumBins - 1));
			// Determine finishing bin
			FVector3 lastbin = FVector3::Clamp((primref.bounds.pmax - origin) * invbinsize, firstbin, FVector3(kNumBins - 1, kNumBins - 1, kNumBins - 1));
			// Iterate over axis
			for (int axis = 0; axis < 3; ++axis)
			{
