...
namespace MOON {
	...
	{
		GizmoObject::ClassMemberCallbackBase* Callable=nullptr;

	public:
		static ObjectCommandInternal* New() { return new ObjectCommandInternal(); }
		void Execute(GizmoObject* caller, unsigned long eventId, void* callData) override
		{
			if (this->Callable)
			{
				this->AbortFlagOff();
		...
		void SetCallable(GizmoObject::ClassMemberCallbackBase* callable)
		{
	...
	ExecuteCommandPair GizmoObject::AddTemplatedObserver(unsigned long event, ClassMemberCallbackBase* callable, float priority)
	{
		//have memory leaks!
