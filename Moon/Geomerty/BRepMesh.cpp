...
struct MeshVertex
{
    ...
    explicit MeshVertex(const Vector3d& p)
        : p(p)
    {
    }

    Vector3d toPoint() const
    {
        return p;
    }
...
public:
    ...
    MergeVertex(std::vector<Vector3d> points,
...
private:
    ...
    void check()
    {
        using VertexIterator = std::vector<Vector3d>::const_iterator;
                                   ...
                                   std::vector<Facet>& faces)
{
    ...
    auto addVertex = [&vertices](const Vector3d& pnt, uint32_t& pointIndex) {
...
std::vector<BRepMesh::Segment> BRepMesh::createSegments() const
{
    ...
    for (size_t numDomainFaces : domainSizes) {
        ...
        std::generate(segment.begin(),
                      segment.end(),
                      iotaGen<std::size_t>(numMeshFaces));
