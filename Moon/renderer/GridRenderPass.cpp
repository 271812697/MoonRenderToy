...
{
	/* Grid Material */
	m_gridMaterial.SetShader(OvCore::Global::ServiceLocator::Get<OvEditor::Core::Context>().editorResources->GetShader("Grid"));
...
void OvEditor::Rendering::GridRenderPass::Draw(OvRendering::Data::PipelineState p_pso)
{
	...
	m_renderer.GetFeature<DebugModelRenderFeature>()
		.DrawModelWithSingleMaterial(pso, *OvCore::Global::ServiceLocator::Get<OvEditor::Core::Context>().editorResources->GetModel("Plane"), m_gridMaterial, model);
