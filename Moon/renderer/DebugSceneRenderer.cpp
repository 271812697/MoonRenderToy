...
namespace
{
	...
	OvMaths::FMatrix4 CreateDebugDirectionalLight()
	{
		...
		directionalLight.type = OvRendering::Settings::ELightType::DIRECTIONAL;
	...
	{
		OvRendering::Entities::Light light;
		light.intensity = 0.01f;
		light.constant = 10000.0f;
		light.type = OvRendering::Settings::ELightType::AMBIENT_SPHERE;
	...
	std::unique_ptr<OvRendering::HAL::ShaderStorageBuffer> CreateDebugLightBuffer()
	{
		...
		OvMaths::FMatrix4 lightMatrices[2] = {
			CreateDebugDirectionalLight(),
			CreateDebugAmbientLight() };

		lightBuffer->Allocate(sizeof(OvMaths::FMatrix4) * 2, OvRendering::Settings::EAccessSpecifier::STATIC_READ);
...
public:
	DebugCamerasRenderPass(OvRendering::Core::CompositeRenderer& p_renderer) : OvRendering::Core::ARenderPass(p_renderer)
	{
		...
		m_cameraMaterial.SetShader(OvCore::Global::ServiceLocator::Get<OvEditor::Core::Context>().shaderManager[":Shaders\\Standard.ovfx"]);
...
protected:
	virtual void Draw(OvRendering::Data::PipelineState p_pso) override
	{
		...
		for (auto camera : sceneDescriptor.scene.GetFastAccessComponents().cameras)
		{
			...
			if (actor.IsActive())
			{
				auto& model = *::OvCore::Global::ServiceLocator::Get<OvEditor::Core::Context>().editorResources->GetModel("Camera");
...
public:
	DebugReflectionProbesRenderPass(OvRendering::Core::CompositeRenderer& p_renderer) : OvRendering::Core::ARenderPass(p_renderer)
	{
		...
		m_reflectiveMaterial.SetShader(OvCore::Global::ServiceLocator::Get<OvEditor::Core::Context>().shaderManager[":Shaders\\Standard.ovfx"]);
...
protected:
	virtual void Draw(OvRendering::Data::PipelineState p_pso) override
	{
		...
		for (auto reflectionProbe : sceneDescriptor.scene.GetFastAccessComponents().reflectionProbes)
		{
			...
			if (actor.IsActive())
			{
				auto& model = *::OvCore::Global::ServiceLocator::Get<::OvEditor::Core::Context>().editorResources->GetModel("Sphere");
				auto modelMatrix =
...
protected:
	virtual void Draw(OvRendering::Data::PipelineState p_pso) override
	{
		...
		for (auto light : sceneDescriptor.scene.GetFastAccessComponents().lights)
		{
			...
			if (actor.IsActive())
			{
				auto& model = *OvCore::Global::ServiceLocator::Get<OvEditor::Core::Context>().editorResources->GetModel("Vertical_Plane");
				...
				auto lightTexture =
					...
					OvCore::Global::ServiceLocator::Get<OvEditor::Core::Context>().editorResources->GetTexture(lightTypeTextureName.value()) :
					nullptr;
...
protected:
	...
	void DrawBoundingSpheres(OvCore::ECS::Components::CModelRenderer& p_modelRenderer)
	{
		...
		if (auto model = p_modelRenderer.GetModel())
		{
			...
			if (frustumBehaviour == OvCore::ECS::Components::CModelRenderer::EFrustumBehaviour::MESH_BOUNDS)
			{
			...
			{
				drawBounds(
					frustumBehaviour == OvCore::ECS::Components::CModelRenderer::EFrustumBehaviour::CUSTOM_BOUNDS ?
	...
	OvCore::Rendering::SceneRenderer(p_driver, true /* enable stencil write, required by the grid */)
{
	...
	AddFeature<OutlineRenderFeature, OvRendering::Features::EFeatureExecutionPolicy::NEVER>();
	AddFeature<GizmoRenderFeature, OvRendering::Features::EFeatureExecutionPolicy::NEVER>();
