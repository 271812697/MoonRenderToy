...
namespace TEST {
	...
	public:
		...
		using MaterialDomain = TEST::MaterialDomain;
		using RefractionMode = TEST::RefractionMode;
		using RefractionType = TEST::RefractionType;
		using ReflectionMode = TEST::ReflectionMode;
		using VertexAttribute = TEST::VertexAttribute;

		using ShaderQuality = TEST::ShaderQuality;
		using BlendingMode = TEST::BlendingMode;
		using BlendFunction = BlendFunction;
		using Shading = TEST::Shading;
		using Interpolation = TEST::Interpolation;
		using VertexDomain = TEST::VertexDomain;
		using TransparencyMode = TEST::TransparencyMode;
		using SpecularAmbientOcclusion = TEST::SpecularAmbientOcclusion;
		...
		using ParameterPrecision = Precision;
		using Precision = Precision;
		using CullingMode = CullingMode;
		using FeatureLevel = FeatureLevel;
		// using StereoscopicType = StereoscopicType;
		...
		MaterialBuilder& buffer(TEST::BufferInterfaceBlock bib) noexcept;
		 ...
		 * @ref TEST::MaterialInstance::setMaskThreshold "MaterialInstance::setMaskThreshold".
		 */
		 ...
		 * @ref TEST::MaterialInstance::setSpecularAntiAliasingVariance
		 * "MaterialInstance::setSpecularAntiAliasingVariance" and
		 * @ref TEST::MaterialInstance::setSpecularAntiAliasingThreshold
	   ...
	   // MaterialBuilder& stereoscopicType(StereoscopicType stereoscopicType) noexcept;
		 ...
		 * limited to 4 morph targets. See TEST::RenderableManager::Builder::morphing().
		 */
	...
	public:
		...
		static constexpr size_t MATERIAL_PROPERTIES_COUNT = TEST::MATERIAL_PROPERTIES_COUNT;
		using Property = TEST::Property;

		using PropertyList = bool[MATERIAL_PROPERTIES_COUNT];
		using VariableList = CustomVariable[MATERIAL_VARIABLES_COUNT];
		using OutputList = std::vector<Output>;

		static constexpr size_t MAX_COLOR_OUTPUT = MRT::MAX_SUPPORTED_RENDER_TARGET_COUNT;
	   ...
	   // std::string peek(ShaderStage type,
		...
		using BufferList = std::vector<std::unique_ptr<TEST::BufferInterfaceBlock>>;
		...
		// TEST::UserVariantFilterMask getVariantFilter() const { return mVariantFilter; }
	...
	private:
	   ...
	   // bool findProperties(ShaderStage type,
	   ...
	   //         CodeGenParams const& semanticCodeGenParams) noexcept;
		...
		bool isLit() const noexcept { return mShading != TEST::Shading::UNLIT; }
