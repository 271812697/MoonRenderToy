...
namespace TEST {
	...
	static bool processBufferField(BufferInterfaceBlock::Builder& builder,
	...
	static bool processBuffer(MaterialBuilder& builder,
		...
		BufferInterfaceBlock::Builder bibb;

		bibb.target(BufferInterfaceBlock::Target::SSBO);
		bibb.alignment(BufferInterfaceBlock::Alignment::std430);
		...
		for (auto value : qualifiersValue->toJsonArray()->getElements()) {
			if (value->getType() == JsonishValue::Type::STRING) {
				...
				using Qualifier = BufferInterfaceBlock::Qualifier;
