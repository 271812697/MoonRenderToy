...
#include <test/math/mat4.h>
#include <test/math/vec4.h>
...
namespace TEST {
	...
	namespace std140 {
		...
		struct mat44 : public std::array< vec4, 4> {
			mat44& operator=(TEST::math::mat4f const& rhs) noexcept {
				for (int i = 0; i < 4; i++) {
					(*this)[i][0] = rhs[i][0];
					(*this)[i][1] = rhs[i][1];
					(*this)[i][2] = rhs[i][2];
	...
	struct PerViewUib { // NOLINT(cppcoreguidelines-pro-type-member-init)
		...
		TEST::math::mat4f viewFromWorldMatrix;    // clip    view <- world    : view matrix
