...
template class utils::PrivateImplementation<TEST::ostream_>;
...
namespace TEST {
	...
	ostream& ostream::print(const char* format, ...) noexcept {
		...
		va_copy(args1, args0);
		int const s = vsnprintf(nullptr, 0, format, args0);
		va_end(args0);
	...
	void ostream::Buffer::advance(int n) noexcept {
		if (n > 0) {
