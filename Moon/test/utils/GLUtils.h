...
#   define CHECK_GL_FRAMEBUFFER_STATUS( target) { GLUtils::checkFramebufferStatus(target, __func__, __LINE__); }
		...
		constexpr inline GLbitfield getAttachmentBitfield(TargetBufferFlags flags) noexcept {
			...
			if (std::underlying_type_t<TargetBufferFlags>(flags) & std::underlying_type_t<TargetBufferFlags>(TargetBufferFlags::COLOR_ALL)) {
				mask |= (GLbitfield)GL_COLOR_BUFFER_BIT;
			}
			if (std::underlying_type_t<TargetBufferFlags>(flags) & std::underlying_type_t<TargetBufferFlags>(TargetBufferFlags::DEPTH)) {
				mask |= (GLbitfield)GL_DEPTH_BUFFER_BIT;
			}
			if (std::underlying_type_t<TargetBufferFlags>(flags) & std::underlying_type_t<TargetBufferFlags>(TargetBufferFlags::STENCIL)) {
