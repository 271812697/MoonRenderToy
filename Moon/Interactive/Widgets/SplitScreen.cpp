...
namespace MOON {
	...
	SplitScreen::SplitScreen(const std::string& name) :EventWidget(name)
	...
	{
		Eigen::Vector2f a = { m_internal->a.x(),renderer->getFrameParam().viewportHeight - m_internal->a.y() };
		Eigen::Vector2f b = { m_internal->b.x(),renderer->getFrameParam().viewportHeight - m_internal->b.y() };
