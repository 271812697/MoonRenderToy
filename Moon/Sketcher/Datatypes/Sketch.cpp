    ...
    if (doesBlockAffectOtherConstraints) {
        ...
        while (unsatisfied_groups) {
            ...
            if (debugMode == GCS::IterationLevel) {
                CORE_INFO("Sketcher::setUpSketch()-BlockConstraint-PostAnalysis:{}\n",index);
...
int Sketch::addConstraints(const std::vector<Constraint*>& ConstraintList)
{
    ...
    for (auto it = ConstraintList.cbegin(); it != ConstraintList.cend(); ++it, ++cid) {
        ...
        if (rtn == -1) {
            ...
            CORE_ERROR("Sketcher constraint number {} is malformed!\n",humanConstraintId);
...
int Sketch::addPerpendicularConstraint(int geoId1, int geoId2)
{
    ...
    CORE_WARN("Perpendicular constraints between {} and {} are not supported.\n",
...
)
{
    ...
    if (knotindex >= b.knots.size()) {
        CORE_ERROR("addConstraint: Knot index out-of-range!\n");
        return -1;
    }
...
)
{
    ...
    if (Geoms[geoId1].type == Point || Geoms[geoId2].type == Point) {
        CORE_ERROR("addAngleAtPointConstraint: one of the curves is a point!\n");
        return -1;
    }
    ...
    if (!crv1 || !crv2) {
        CORE_ERROR("addAngleAtPointConstraint: getGCSCurveByGeoId returned NULL!\n");
        return -1;
    }
    ...
    if (pointId < 0 || pointId >= int(Points.size())) {
        CORE_ERROR("addAngleAtPointConstraint: point index out of range.\n");
        return -1;
    }
    GCS::Point& p = Points[pointId];
...
)
{
    ...
    if (Geoms[geoIdRay1].type == Point || Geoms[geoIdRay2].type == Point) {
        CORE_ERROR("addSnellsLawConstraint: point is not a curve. Not applicable!\n");
        return -1;
    }
    ...
    if (!ray1 || !ray2 || !boundary) {
        CORE_ERROR("addSnellsLawConstraint: getGCSCurveByGeoId returned NULL!\n");
        return -1;
    }
    ...
    if (pointId1 < 0 || pointId1 >= int(Points.size()) || pointId2 < 0
        || pointId2 >= int(Points.size())) {
        CORE_ERROR("addSnellsLawConstraint: point index out of range.\n");
        return -1;
    }
    GCS::Point& p1 = Points[pointId1];
...
bool Sketch::updateGeometry()
{
    ...
    for (const GeoDef& it : Geoms) {
        try {
            ...
            CORE_ERROR("Updating geometry: Error build geometry({}): {}\n", i, e.what());
            return false;
        }
    }
    return true;
...
int Sketch::internalSolve(std::string& solvername, int level)
{
    ...
    if (ret == GCS::Success) {
        ...
        if (!valid_solution) {
            GCSsys.undoSolution();
            updateGeometry();
            CORE_WARN("Invalid solution from {} solver.\n", solvername.c_str());
        }
        else {
        ...
        if (debugMode == GCS::Minimal || debugMode == GCS::IterationLevel) {
            ...
            CORE_WARN("Sketcher::Solve()-{}- Failed!! Falling back...\n", solvername.c_str());
        }
    }
