...
BufferInterfaceBlock::Builder::name(std::string interfaceBlockName) {
...
BufferInterfaceBlock::Builder& BufferInterfaceBlock::Builder::add(
    ...
    for (auto const& item : list) {
        mEntries.push_back({
                ...
                0, uint8_t(item.stride), item.type, item.size > 0, item.size, item.precision, item.associatedSampler,
...
BufferInterfaceBlock::Builder& BufferInterfaceBlock::Builder::addVariableSizedArray(
    ...
    mEntries.push_back({
            { item.name.data(), item.name.size() },
            0, uint8_t(item.stride), item.type, true, 0, item.precision, item.associatedSampler,
            { item.structName.data(), item.structName.size() },
            { item.sizeName.data(), item.sizeName.size() }
    });
    return *this;
...
BufferInterfaceBlock BufferInterfaceBlock::Builder::build() {
    ...
    assert((pos == mEntries.end() || pos == mEntries.end() - 1) && "the variable-size array must be the last entry");

    // if we have a variable size array, we can't be a UBO
    assert((pos == mEntries.end() || mTarget == Target::SSBO) && "variable size arrays not supported for UBOs");

    // std430 not available for UBOs
    assert((mAlignment == Alignment::std140 || mTarget == Target::SSBO) && "UBOs must use std140");
      ...
      mTarget(builder.mTarget)
{
    ...
    for (auto const& e : builder.mEntries) {
        ...
        info = { e.name, offset, uint8_t(stride), e.type, e.isArray, e.size,
                 e.precision, e.associatedSampler,e.structName, e.sizeName };
...
size_t BufferInterfaceBlock::getFieldOffset(std::string name, size_t index) const
{
