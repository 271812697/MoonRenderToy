...
int main(int argc, char* argv[]) {
    ...
    for (const auto& inPath : inputPaths) {
        ...
        ifstream inStream(inPath.getPath(), ios::binary | ios::ate);
        if( !inStream )
        {
        ...
        std::string rname = inPath.getNameWithoutExtension()+inPath.getExtension();
        std::string filename = inPath.getName();
