...
int main(int argc, char* argv[]) {
    ...
    headerStream << "#ifndef " << packagePrefix << "H_" << endl
            ...
            << endl <<"extern std::unordered_map<std::string, int>" << packagePrefix << "OFFSET;" << endl
            << endl
                 << "extern std::unordered_map<std::string, int>" << packagePrefix << "LEN;" << endl
            << endl
                 << "const uint8_t* " << g_packageName << "Package();" << endl
            << "extern \"C\" {" << endl
    ...
    if (g_generateC) {
        ...
        xxdStream << "std::unordered_map<std::string, int>" << packagePrefix << "OFFSET;" << endl;
        xxdStream << "std::unordered_map<std::string, int>" << packagePrefix << "LEN;" << endl;
    ...
    for (const auto& inPath : inputPaths) {
        ...
        std::string filename = inPath.getNameWithoutExtension();
        ...
        if (g_generateC) {
            ...
            func << "    " << packagePrefix << "OFFSET[\"" << filename << "\"]=" << offset << ";" << endl;
            func << "    " << packagePrefix << "LEN[\"" << filename << "\"]=" << content.size() << ";" << endl;
            xxdStream << "// " << rname << "\n";
    ...
    func << "const uint8_t* "<< g_packageName << "Package(){" << endl
    ...
    if (!g_quietMode) {
        cout << "Generated files: " << headerPath << " ";
