...
void MaterialTextChunk::writeEntryAttributes(size_t entryIndex, Flattener& f) const noexcept {
    ...
    f.writeUint8(uint8_t(entry.passTag));
    f.writeUint8(entry.variant);
    f.writeUint8(uint8_t(entry.stage));
}

void compressShader(std::string src, Flattener &f, const LineDictionary& dictionary) {
    ...
    while (s[cur] != '\0'&&cur<src.size()) {
        ...
        if (index > UINT16_MAX) {
            assert(false&&"Dictionary returned line index > UINT16_MAX");
...
void MaterialTextChunk::flatten(Flattener& f) {
    ...
    if (mDuplicateMap.empty()) {
        ...
        for (size_t i = 0; i < mEntries.size(); i++) {
            ...
            auto iter = stringToIndex.find(text);
