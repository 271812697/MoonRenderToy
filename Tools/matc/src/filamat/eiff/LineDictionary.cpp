...
std::string LineDictionary::getString(size_t index) const noexcept {
...
size_t LineDictionary::getIndex(std::string s) const noexcept {
    auto iter = mLineIndices.find(s);
    if ( iter != mLineIndices.end()) {
        return iter->second;
    }
    return SIZE_MAX;
}
