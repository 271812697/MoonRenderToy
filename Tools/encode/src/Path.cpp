...
Path Path::getCurrentDirectory() {
	...
	if (_getcwd(directory, MAX_PATH) == nullptr) {
		return Path();
	}
...
std::string Path::getReleativeTo(const std::string& parent)const 
{
    std::string temp = getCanonicalPath(parent);
    if( m_path.substr(0, temp.size()) != temp)
    {
        return "Error";
	}
    std::string res = m_path.substr(temp.size()).substr(1);
