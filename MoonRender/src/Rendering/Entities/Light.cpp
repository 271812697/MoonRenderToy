...
namespace
{
	...
	)
	{
		...
		renderTexture->SetBorderColor(Maths::FVector4::One);
	...
	Rendering::Entities::Camera CreateCamera(
		const Rendering::Entities::Light& p_light,
		const Rendering::Data::FrameDescriptor& p_frameDescriptor
	)
	{
		assert(
		...
		const auto lightPosition =
			...
			p_frameDescriptor.camera->transform->GetWorldPosition() + Maths::FVector3::Up * p_light.shadowAreaSize :  // offsets the camera position be positioned on the top plane of the effect area
			p_light.transform->GetWorldPosition();

		Rendering::Entities::Camera lightCamera;
		lightCamera.SetNear(0.1f);
		lightCamera.SetFar(p_light.shadowAreaSize * 2.0f); // x2 because the light will be positioned on the top plane of the effect area (abe the camera)
		lightCamera.SetSize(p_light.shadowAreaSize);
		lightCamera.SetProjectionMode(Rendering::Settings::EProjectionMode::ORTHOGRAPHIC);
...
void Rendering::Entities::Light::PrepareForShadowRendering(const Rendering::Data::FrameDescriptor& p_frameDescriptor)
{
	assert(type == Settings::ELightType::DIRECTIONAL && "Cannot prepare shadow buffer for a non directional light! (not supported yet)");

	shadowCamera = CreateCamera(*this, p_frameDescriptor);

	if (!shadowBuffer)
	{
		shadowBuffer = std::make_unique<Rendering::HAL::Framebuffer>("DirectionalShadow");
	...
	assert(IsSetupForShadowRendering()&&"Light failed to setup for shadow rendering!");
}

bool Rendering::Entities::Light::IsSetupForShadowRendering() const
{
	return
...
float Rendering::Entities::Light::CalculateEffectRange() const
{
	switch (type)
	{
