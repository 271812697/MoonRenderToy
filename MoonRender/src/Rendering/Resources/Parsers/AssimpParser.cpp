...
namespace
{
	Rendering::Resources::Parsers::EModelParserFlags FixFlags(Rendering::Resources::Parsers::EModelParserFlags p_flags)
	{
		...
		{
			p_flags &= ~GEN_NORMALS;
			("AssimpParser: GEN_NORMALS and GEN_SMOOTH_NORMALS are mutually exclusive. GEN_NORMALS will be ignored.");
		}

		if (static_cast<bool>(p_flags & OPTIMIZE_GRAPH) && static_cast<bool>(p_flags & PRE_TRANSFORM_VERTICES))
		{
			p_flags &= ~OPTIMIZE_GRAPH;
			("AssimpParser: OPTIMIZE_GRAPH and PRE_TRANSFORM_VERTICES are mutually exclusive. OPTIMIZE_GRAPH will be ignored.");
		}
