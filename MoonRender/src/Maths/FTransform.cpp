...
void Maths::FTransform::NotificationHandler(Internal::TransformNotifier::ENotification p_notification)
{
	...
	case Internal::TransformNotifier::ENotification::TRANSFORM_DESTROYED:
		/*
		* RemeParent() is not called here because it is unsafe to reme a notification handler
...
void Maths::FTransform::LookAt(const FVector3& eye, const FVector3& center)
{
	float pi = 3.14159265359f;
	FVector3 forward = FVector3::Normalize(center - eye);
	float angle = Maths::FVector3::AngleBetween(forward, { 0,1,0 });
	FVector3 up = (angle < FLT_EPSILON || abs(angle - pi) < FLT_EPSILON) ? Maths::FVector3(1, 0, 0) : Maths::FVector3(0, 1, 0);
