...
#include "Geometry.h"
#include "App/BSplineCurveBiArcs.h"
...
GeomBSplineCurve* GeomCircle::toNurbs(double first, double last) const
{
    ...
    if (first != 0 || last != 2 * 3.141592653589793) {
    ...
    knots(1) = 0;
    knots(2) = 3.141592653589793;
    knots(3) = 2 * 3.141592653589793;
...
gp_Vec GeomCone::getDN(double u, double v, int Nu, int Nv) const
{
    ...
    auto ElSLib__ConeDN = [](const Standard_Real U,
        ...
        Standard_Real Um = U + Nu * 3.141592653589793 / 2;
...
)
{
    ...
    if (endAngle > 2 * 3.141592653589793) {
...
)
{
    if (geo1->is<GeomLineSegment>() && geo2->is<GeomLineSegment>()) {
        ...
        if (fmod(fabs(range), 2 * 3.141592653589793) < Precision::Approximation()) {
            return nullptr;
        }
