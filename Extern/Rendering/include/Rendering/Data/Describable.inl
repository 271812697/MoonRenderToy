...
namespace Rendering::Data
{
	...
	{
		ASSERT(!HasDescriptor<T>(), "Descriptor already added");
	...
	{
		ASSERT(!HasDescriptor<T>(), "Descriptor doesn't exist.");
	...
	{
		auto it = m_descriptors.find(typeid(T));
		ASSERT(it != m_descriptors.end(), "Couldn't find a descriptor matching the given type T.");
