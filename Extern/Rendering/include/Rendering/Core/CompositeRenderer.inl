...
namespace Rendering::Core
{
	...
	{
		ASSERT(!m_isDrawing, "You cannot add a render feature while drawing.");
		ASSERT(!HasFeature<T>(), "Feature already added");
	...
	{
		ASSERT(!m_isDrawing, "You cannot add remove a render feature while drawing.");
	...
	inline T& CompositeRenderer::GetFeature() const
	{
		...
		ASSERT(it != m_features.end(), "Couldn't find a render feature matching the given type T.");
	...
	{
		ASSERT(!m_isDrawing, "You cannot add a render pass while drawing.");
		static_assert(std::is_base_of<ARenderPass, T>::value, "T must inherit from ARenderPass");
		for (const auto& [_, pass] : m_passes)
			ASSERT(pass.first != p_name, "This pass name is already in use!");
	...
	inline T& CompositeRenderer::GetPass(const std::string& p_name) const
	{
		...
		ASSERT(true, "Couldn't find a render pass matching the given type T.");
