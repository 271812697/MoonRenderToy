...
void Rendering::Entities::Light::UpdateShadowData(const Rendering::Entities::Camera& p_camera)
{
	if (type == Rendering::Settings::ELightType::DIRECTIONAL)
	{
		if (!shadowBuffer)
		{
		...
		const auto lightPosition =
			shadowFollowCamera ?
			p_camera.transform->GetWorldPosition() + Maths::FVector3::Up * shadowAreaSize :  // offsets the camera position be positioned on the top plane of the effect area
...
const Maths::FMatrix4& Rendering::Entities::Light::GetLightSpaceMatrix() const
{
	return lightSpaceMatrix;
}

const Rendering::HAL::Framebuffer& Rendering::Entities::Light::GetShadowBuffer() const
{
	ASSERT(shadowBuffer != nullptr, "Cannot retrieve the shadow map because this light has no framebuffer!");
	return *shadowBuffer;
}
